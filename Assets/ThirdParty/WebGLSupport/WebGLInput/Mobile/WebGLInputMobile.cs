...
namespace WebGLSupport
{
    ...
    public class WebGLInputMobile : MonoBehaviour, IPointerDownHandler
    {
        ...
        static void OnTouchEnd(int id)
        {
            ...
            //GlobalEventRegistry.Invoke(GameSetting.ShowHollowOutMask, null, new HollowOutEventArg(null, () => { OnFocusOut(id); }, true));
            @this.GetComponent<WebGLInput>().OnSelect();
            DelayCallback.BeginCoroutine(RegisterOnFocusOut(id));
        ...
        {
            //GlobalEventRegistry.Invoke(GameSetting.HideHollowOutMask, null, EventArgs.Empty);
            //var @this = instances[id];
            DelayCallback.BeginCoroutine(ExecFocusOut(id));
